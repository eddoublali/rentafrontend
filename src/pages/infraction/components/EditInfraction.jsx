...
export default function EditInfraction() {
  ...
  const { id } = useParams();
  const { updateInfraction, error: infractionError, getOneInfraction } = useInfraction();
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        await Promise.all([fetchClients(), fetchVehicles()]);
        
        
        const numericId = parseInt(id, 10);
        
        const infractionData = await getOneInfraction(numericId);
        console.log(infractionData.data)
        if (infractionData && infractionData.data) {
          const infraction = infractionData.data;
          
         
          const formattedDate = infraction.infractionDate 
            ? new Date(infraction.infractionDate).toISOString().split('T')[0]
            : new Date().toISOString().split('T')[0];
            
          setFormData({
            clientId: infraction.clientId || '',
            vehicleId: infraction.vehicleId || '',
            infractionType: infraction.infractionType || '',
            fineAmount: infraction.fineAmount || 0,
            infractionDate: formattedDate,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await updateInfraction(id, formattedData);
      
      
      console.log('Update infraction result:', response);
      
      if (!response?.success) {
        console.error('Failed to update infraction:', response?.message);
        setErrors({
          ...errors,
          general: response?.message || t('errors.failedToUpdateInfraction'),
        });
        return;
      }
      
      console.log('Infraction updated successfully:', response?.data);
      navigate('/infractions');
    } catch (err) {
      console.error('Unexpected error:', err);
      setErrors({
        ...errors,
        general: t('errors.failedToUpdateInfraction'),
      });
    } finally {
      setIsSaving(false);
    }
  ...
  if (notFound) {
    return (
      <div className="py-10 text-gray-500 items-center p-6 text-center">
        <h1 className="text-xl font-semibold text-error">
          {t("common.infractionNotFound")}
        </h1>
        <p className="mt-2">{t("infraction.notFoundMessage") || "The requested infraction could not be found."}</p>
        ...
        <p className="mt-2">
          ...
          >
            <ArrowLeft size={16} /> {t("infraction.return") || "Return to Infractions"}
          </button>
        </p>
      </div>
    );
  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-2xl font-bold">{t("infraction.editInfraction")}</h2>
        <button
          type="button"
          onClick={() => navigate('/infractions')}
      ...
      >
        ...
        <div className="col-span-1 md:col-span-2 mt-10">
          ...
          >
            {isSaving ? t('common.saving') : t('infraction.updateInfraction')}
          </button>
        </div>
      </form>
    </div>
